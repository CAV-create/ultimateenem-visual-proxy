...
```bash
DROPBOX_ACCESS_TOKEN=sl_xxxxxxxxxxxxxxxxx
...
Use o token `sl...` do Dropbox enquanto estamos testando. Para producao permanente, trocar por fluxo OAuth com refresh token.
...
5. No schema `openapi_gpt_action_visual_proxy.yaml`, troque:

```yaml
servers:
  - url: https://SEU-DOMINIO-DO-PROXY
```
